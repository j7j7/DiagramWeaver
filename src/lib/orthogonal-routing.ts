...
const ROUTING_CORRIDOR_STUB = CELL * 3;

/**
 * Envelope of anchor points the router uses (stubs, waypoints). Any orthogonal
 * detour that stays in-bounds is contained in this AABB with asymmetric padding
 * (wide sideways band + tall vertical band) so we can drop far-away obstacles.
 */
function routingAnchorEnvelope(
  fromX: number,
  fromY: number,
  toX: number,
  toY: number,
  ...
  const pts: { x: number; y: number }[] = [
    exitStub(fromX, fromY, fromAngle, ROUTING_CORRIDOR_STUB),
    exitStub(toX, toY, toAngle, ROUTING_CORRIDOR_STUB),
  ];
  if (waypoints?.length) {
    for (const w of waypoints) pts.push({ x: w.x, y: w.y });
  }
  let minX = pts[0].x;
  let maxX = pts[0].x;
  let minY = pts[0].y;
...
/** Pixels added when growing the envelope into a filter rect (A* can jog sideways / vertically). */
const CORRIDOR_PAD_X = 500;
const CORRIDOR_PAD_Y = 500;

/**
 * Remove obstacles that cannot affect routing between the two endpoints (and optional
 * waypoints). Conservative: if the filter would remove every obstacle, returns `obstacles` unchanged.
 */
export function cullDistantObstaclesForConnectionCorridor(
  obstacles: Rect[],
  fromX: number,
  fromY: number,
  toX: number,
  ...
  if (obstacles.length < 2) return obstacles;

  const { minX, maxX, minY, maxY } = routingAnchorEnvelope(
    fromX,
    fromY,
    toX,
    toY,
  ...
  const minBX = minX - CORRIDOR_PAD_X - spanY * 0.5;
  const maxBX = maxX + CORRIDOR_PAD_X + spanY * 0.5;
  const minBY = minY - CORRIDOR_PAD_Y - spanX * 0.5;
  const maxBY = maxY + CORRIDOR_PAD_Y + spanX * 0.5;
  ...
  for (const o of obstacles) {
    ...
    if (omidX >= minBX && omidX <= maxBX && omidY >= minBY && omidY <= maxBY) {
      out.push(o);
    }
  }
