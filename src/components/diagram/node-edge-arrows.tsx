...
export function NodeEdgeArrows({ node, connections, width, height }: NodeEdgeArrowsProps) {
  const isTextNode = !isIconOrEmojiType(node.type) && (node.type === 'generic.text.text'  ||
                      ...
                      node.type === 'generic.object.point' || node.type === 'generic.object.rectangle' || node.type === 'generic.object.rounded-rectangle' || node.type === 'generic.object.progress-bar' || node.type === 'generic.object.text-box-heading' || node.type === 'generic.object.triangle' ||
                      node.type === 'generic.object.star' || node.type === 'generic.object.cloud' ||
                      node.type?.endsWith('.square') || node.type?.endsWith('.circle') ||
                      node.type?.endsWith('.point') || node.type?.endsWith('.rectangle') || node.type?.endsWith('.rounded-rectangle') || node.type?.endsWith('.progress-bar') || node.type?.endsWith('.text-box-heading') || node.type?.endsWith('.triangle') ||
