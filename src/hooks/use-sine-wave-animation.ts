...
export function useNodeAnimationOffsets(nodes: any[], selectedIds: Set<string>) {
  ...
  useEffect(() => {
    const newPhases: Record<string, { phaseX: number; phaseY: number; freqX: number; freqY: number }> = { ...randomPhases };
    
    nodes.forEach(node => {
      if (selectedIds.has(node.id) && !randomPhases[node.id]) {
      ...
      } else if (!selectedIds.has(node.id) && randomPhases[node.id]) {
  ...
  const updateOffsets = useCallback(() => {
    ...
    nodes.forEach(node => {
      if (selectedIds.has(node.id)) {
        const phase = randomPhases[node.id];
        if (phase) {
  ...
  }, [nodes, selectedIds, randomPhases]);
