...
- **`useViewportRenderCull`** / **`computeViewportRenderCull`** in **`src/lib/viewport-culling.ts`**: from pan/zoom + host size, only mount **`DiagramNode`** instances whose bounds intersect the padded view (**32px** screen margin, **48** diagram-unit cap); connections use **16px** tests on endpoint boxes **or** path segments (centers + waypoints; orthogonal also samples chord + simple Z trunks) so mid-path zooms keep lines while empty zoomed-out corners stay culled. Pass **`connectionIndices`** into **`CanvasConnections`** / **`CanvasConnectionText`**. Active when the diagram has any items or connections; disabled during GIF export. Selected, dragged, hovered, and selected-edge endpoints are always included.
