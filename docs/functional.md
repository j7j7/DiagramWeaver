...
1. **New node shape**: add resource or type string in catalog, implement component under **`components/diagram/shapes/`**, register in **`diagram-node`** / type switches, extend **`types.ts`** + **`schemas.ts`** if new fields are persisted. Charts: pie/bar/line/ring/grid plus **Gantt** (`generic.chart.gantt`, **`gantt-chart-layout.ts`**), **Loop** (`generic.chart.loop`, **`loop-chart-layout.ts`**), and **Arrow** (`generic.chart.arrow`, **`arrow-chart-layout.ts`**).
